        ...
        if response:
            ...
            try:
                ...
                if start != -1 and end != -1:
                    ...
                    tgt = data.get('file', 'agent.py')
                    if change and not change.startswith('{'):
                        return f"FILE: {tgt} | {change[:100]} | {reason[:60]}"
            except Exception:
                pass
            # Fallback: strip JSON blobs, take first plain sentence
            clean = response.strip().split('\n')[0]
            import re as _re; clean = re.sub(r'\{.*?\}', '', clean).strip(' ,"')
            if clean and len(clean) > 10:
                return f"FILE: agent.py | {clean[:100]}"
...
Under 30 lines. End with a comment: # STATUS: continue or # STATUS: complete"""
    ...
    def implement_goal_step(self, goal: str, target_file: str = "agent.py") -> str:
        """OpenClaw-style: read target file, generate a concrete patch, apply + commit.
        Returns a summary string or error description."""
        import ast as _ast, shutil as _sh
        from pathlib import Path as _Path

        allowed = {'agent.py', 'inference.py', 'tools.py', 'planner.py', 'comms.py'}
        if target_file not in allowed:
            return f"Blocked: {target_file} not in allowed set"

        target_path = _Path(self.repo_path) / target_file
        try:
            original = target_path.read_text(encoding='utf-8')
        except Exception as e:
            return f"Read error: {e}"

        # Give LLM the first 60 lines + last 20 as context window
        lines = original.splitlines()
        context_lines = lines[:60] + ['... (truncated) ...'] + lines[-20:] if len(lines) > 80 else lines
        context = chr(10).join(context_lines)

        prompt = (
            f"You are patching {target_file} for an autonomous Raspberry Pi agent.\n"
            f"Goal: {goal[:150]}\n\n"
            f"Current file (truncated):\n{context[:1200]}\n\n"
            "Write ONE complete Python method that implements the most concrete next step toward this goal.\n"
            "Output ONLY the method definition (def ...). No markdown. No extra text. Under 30 lines.\n"
            "def "
        )
        try:
            raw = self.inference.generate_code(prompt, max_tokens=400)
        except Exception as e:
            return f"LLM error: {e}"

        if not raw or len(raw.strip()) < 20:
            return "Empty or too-short LLM response"

        code = raw.strip()
        if code.startswith('```'):
            code = chr(10).join(l for l in code.splitlines()
        ...
        # Indent to class method level (4 spaces)
        indented = chr(10).join(
            ('    ' + ln if ln.strip() else ln)
            for ln in code.splitlines()
        )

        # Find and replace existing method, or append
        method_pat = re.compile(
            r'(    def ' + re.escape(func_name) + r'\(self.*?)(?=\n    def |\Z)',
            re.DOTALL
        )
        if method_pat.search(original):
            new_content = method_pat.sub(indented, original)
            action = f"replaced {func_name}()"
        else:
            # Append before the last `if __name__` or at end of class
            insert_before = '\nif __name__'
            if insert_before in original:
                new_content = original.replace(insert_before, chr(10) + indented + chr(10) + insert_before)
            else:
                new_content = original.rstrip() + chr(10) + chr(10) + indented + chr(10)
            action = f"added {func_name}()"

        # Final syntax check on patched file
        try:
            _ast.parse(new_content)
        except SyntaxError as e:
            return f"Patched file has SyntaxError: {e}"

        _sh.copy(str(target_path), str(target_path) + ".bak")
        target_path.write_text(new_content, encoding='utf-8')
        logger.info(f"implement_goal_step: {action} in {target_file}")

        # Git commit
        commit_msg = f"agent: goal#{self.state.get('iterations',0)} {action} — {goal[:50]}"
        self.git.commit(commit_msg)
        logger.info(f"implement_goal_step: committed '{commit_msg[:60]}'")
        return f"Applied + committed: {action} in {target_file}"
    ...
    def self_edit_file(self, file_name: str, change_desc: str):
        ...
        func_m = re.search(r'^def (\w+)', new_code)
        if not func_m:
    ...
    def self_modify(self) -> Optional[str]:
        ...
        _func_pat = re.compile(r'(    def think\(self.*?)(?=\n    def )', re.DOTALL)
        try:
    ...
    def run_iteration(self):
        ...
        # Every 5th iteration: implement a concrete step toward the active goal
        if self.state["iterations"] % 5 == 0 and goal:
            # Parse target file from decision if available
            decision_raw = self.state.get("last_think_raw", "")
            tgt_file = "agent.py"
            if decision_raw and decision_raw.startswith("FILE:"):
                tgt_candidate = decision_raw.replace("FILE:", "").split("|")[0].strip()
                if tgt_candidate in {"agent.py", "inference.py", "tools.py", "planner.py", "comms.py"}:
                    tgt_file = tgt_candidate
            impl_result = self.implement_goal_step(goal["description"], tgt_file)
            logger.info(f"implement_goal_step: {impl_result}")
            discord_message += f"🔨 **Implemented:** {impl_result}\n"
        elif self.state["iterations"] % 5 == 0:
        ...
        # Seek improvements (only every 4th iteration — expensive, rate-limited)
        try:
            improvements = self.seek_improvements() if self.state["iterations"] % 4 == 0 else []
